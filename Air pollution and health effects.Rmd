...
event_data \<- cleaned_pollution_data %\>% filter( year \>= 2015 ) %\>% group_by(year) %\>% summarise(avg_pollution = mean(`Air Pollution Population Weighted Average [ug/m3]`, na.rm = TRUE))

Variable 2

# Creates a categorical tracking factor that splits observations around the 2020 COVID event

cleaned_pollution_data \<- cleaned_pollution_data %\>% mutate(period = ifelse(year \< 2020, "Before 2020", "2020 onwards"))

cleaned_pollution_data$period <- factor(
  cleaned_pollution_data$period, levels = c("Before 2020", "2020 onwards") )
...
The covid lockdown analysis is about the change in air pollution before and after the lockdowns of 2020. It could give us a better picture of changes after the event, which could give more depth and future directions in the research of our social problem of tackeling health issues related to air pollution.
